...
public:
	...
	virtual int specialSkill(vector<AbstractMonster*>);
	int attackTo(AbstractMonster*);
	int attackToAll(vector<AbstractMonster*>);

	virtual void status();
	virtual void status_pic();
	string serialize() const;
	void unserialize(string);

	void SetColor(int f = 7, int b = 0);
	void perentbox(double f = 7, double b = 0,int color=7);
